...
function start() {
    ...
    if (cm.getLevel() >= 70 && cm.getJobId() == 310)
        actionx["Mental"] = true;
	else if (cm.getLevel() >= 70 && cm.getJobId() == 320)
        actionx["Mental"] = true;
...
function action(mode, type, selection){
	...
	if (actionx["Mental"]){
		...
		// else if (status == 1)
			cm.sendYesNo("Okay! Now, you'll be transformed into a much more powerful archer through me. Before doing that, though, please make sure your SP has been thoroughly used, You'll need to use up at least all of SP's gained until level 70 to make the 3rd job advancement. Oh, and since you have already chosen your path of the occupation by the 2nd job adv., you won't have to choose again for the 3rd job adv. Do you want to do it right now?");
		else if (status == 1) {
