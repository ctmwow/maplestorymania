    ...
    if (mode < 0)
        ...
        if (status == 0 && mode == 1) {
			if(cm.getPlayer().getLevel() < 20) {
				cm.sendOk("Hello, I am the Vote Point exchanger for #rMaplePhoenix#k!\r\n\r\nI am sorry, but I can only exchange Vote Points for players #blevel 20 or over#k.");
				cm.dispose();
				return;
			}
			var outStr = "Hello, I am the Vote Point exchanger for #rMaplePhoenix#k!\r\n";
			...
			if(choice == 0) {
				...
				cm.getPlayer().announce(Packages.org.ascnet.leaftown.tools.MaplePacketCreator.earnTitleMessage("You have earned " + nxAmount + " NX"));
				cm.logLeaf(nxAmount + " NX");
				cm.dispose();
			} else if(choice == 2) {
				if(!cm.getPlayer().getInventory(Packages.org.ascnet.leaftown.client.inventory.MapleInventoryType.SETUP).isFull(chairAmount)) {
				...
				if(!cm.getPlayer().getInventory(Packages.org.ascnet.leaftown.client.inventory.MapleInventoryType.EQUIP).isFull(weaponAmount)) {
				...
				if(!cm.getPlayer().getInventory(Packages.org.ascnet.leaftown.client.inventory.MapleInventoryType.USE).isFull(2)) {
				...
				if(!cm.haveItem(5030000, 1)) {
					if(!cm.getPlayer().getInventory(Packages.org.ascnet.leaftown.client.inventory.MapleInventoryType.CASH).isFull(1)){
