...
function action(mode, type, selection) {
    if(mode == 0 && status == 0) {
	cm.sendOk("I see. It's very understandable, considering the fact that you'll be facing a very dangerous monster inside. If you ever feel a change of heart, then please come talk to me. I sure can use help from someone like you.");
	cm.dispose();
	return;
	...
	if(status == 0) {
	    ...
	    cm.sendNext("Thank you so much. Actually, #b#p2040001##k asked you to get #b#t4031093##k as a way of testing your abilities to see if you can handle this, so don't think of it as a random request. I think someone like you can handle adversity well.");
	} else if(status == 2) {
    ...
    } else if(cm.getQuestStatus(3230).equals(MapleQuestStatus.Status.COMPLETED)) {
