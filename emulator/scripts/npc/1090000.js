...
function start() {
    ...
    } else if (actionx["3thJobI"] || (cm.getPlayer().gotPartQuestItem("JB3") && cm.getLevel() >= 70 && cm.getJobId() % 10 == 0 && parseInt(cm.getJobId() / 100) == 5 && !cm.getPlayer().gotPartQuestItem("JBP"))){
    ...
    if (actionx["1stJob"]){
      ...
      //  } else if (status == 1){
        //    if (actionx["2ndJobT"]) {
            ...
            cm.sendNext("Alright, you're the " + (job == 510 ? "#bBrawler#k" : "#bGunslinger#k") + " from here on out.");
            if (cm.getJobId() != job)
