...
public class AbstractPlayerInteraction {
    ...
    public boolean canHold(int itemid, boolean fullInvent) {
        if (!fullInvent) {
            if (getPlayer().getInventory(MapleItemInformationProvider.getInstance().getInventoryType(itemid)).getNextFreeSlot() == -1)
                return false;
        } else {
            for (int i = 1; i <= 5; i++) {
    ...
    {
        c.sendPacket(MaplePacketCreator.showTutorActions((byte) 0x00, message, 
        		ByteBuffer.wrap(new byte[] { (byte) 0xC8, 0x00, 0x00, 0x00 }).getInt(), 
        		ByteBuffer.wrap(new byte[] { (byte) 0xA0, (byte) 0x0F, 0x00, 0x00}).getInt()));
    }
