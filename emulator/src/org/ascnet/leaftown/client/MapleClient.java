...
import org.ascnet.leaftown.scripting.npc.NPCConversationManager;
import org.ascnet.leaftown.scripting.npc.NPCScriptManager;
import org.ascnet.leaftown.scripting.quest.QuestActionManager;
...
import org.ascnet.leaftown.tools.IPAddressTool;
import org.ascnet.leaftown.tools.MapleAESOFB;
import org.ascnet.leaftown.tools.MapleCustomEncryption;
...
public class MapleClient extends ChannelInboundHandlerAdapter {
    ...
    private byte characterSlots = 0x03;
    ...
    public int login(String login, String pwd) {
        ...
        Connection con = DatabaseConnection.getConnection();
        try {
            PreparedStatement ps = con.prepareStatement("SELECT id, password, gender, characterslots, pin, pic, banned, tempban, greason, gm, createDate, banreason, macs, forumuserid FROM accounts WHERE name = ?");
    ...
    public static String getChannelServerIPFromSubnet(String clientIPAddress, int channel) {
        ...
        if (subnetInfo.contains("org.ascnet.leaftown.net.login.subnetcount")) {
            int subnetCount = Integer.parseInt(subnetInfo.getProperty("org.ascnet.leaftown.net.login.subnetcount"));
            for (int i = 0; i < subnetCount; i++) {
                String[] connectionInfo = subnetInfo.getProperty("org.ascnet.leaftown.net.login.subnet." + i).split(":");
    ...
    public void updateLoginState(int newstate) {
        if (newstate == 1 || newstate >= 4 && newstate <= 6) {
            if (idleDisconnect != null)
                idleDisconnect.cancel(true);
            idleDisconnect = TimerManager.getInstance().schedule(new Runnable() {
