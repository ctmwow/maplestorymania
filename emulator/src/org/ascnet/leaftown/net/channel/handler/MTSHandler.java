...
public class MTSHandler extends AbstractMaplePacketHandler {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(MTSHandler.class);
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (slea.available() > 0) {
            ...
            if (op == 2) { // put item up for sale
                ...
                short position = (short) slea.readInt();
                slea.skip(4);
                int price = slea.readInt();
                final int taxedPrice = price + 100 + (int) (price * 0.1);
                if (price < 221 || slea.available() != 2 || position > 96 || taxedPrice < 0) {
                    return;
                }

                MapleInventoryType type = MapleItemInformationProvider.getInstance().getInventoryType(itemid);
                IItem i = c.getPlayer().getInventory(type).getItem(position).copy();
                if (i == null || i.getItemId() != itemid || quantity != i.getQuantity()) {
                    return;
                }
                if (ii.getName(itemid).startsWith("Reverse") || ii.getName(itemid).startsWith("Timeless")) { // TODO: Better handling?
                    c.getPlayer().dropMessage(6, "This item cannot be sold here.");
                ...
                if (i.isByGM() && c.getPlayer().isGMLegit()) {
                ...
                try {
                    ...
                    if (rs.next()) {
                        if (rs.getInt(1) > 10) { //They have more than 10 items up for sale already!
                            c.sendPacket(MaplePacketCreator.serverNotice(1, "You already have 10 items up for auction!"));
                    ...
                    if (i.getType() == 2) 
                    {
                        ...
                        ps.setInt(8, item.getFlag());
                        ps.setTimestamp(9, item.getExpiration());
                        ps.setString(10, c.getPlayer().getName());
                        ps.setTimestamp(11, endDate);
                    } else {
                        Equip equip = (Equip) i;
                        ps = con.prepareStatement("INSERT INTO mts_items (tab, type, itemid, quantity, seller, price, upgradeslots, level, str, dex, `int`, luk, hp, mp, watk, matk, wdef, mdef, acc, avoid, hands, speed, jump, owner, flag, ExpireDate, hammer, sellername, sell_ends) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                        ps.setInt(1, 1);
                        ps.setInt(2, (int) type.getType());
                        ...
                        ps.setString(24, equip.getOwner());
                        ps.setInt(25, equip.getFlag());
                        ps.setTimestamp(26, equip.getExpiration());
                        ps.setInt(27, equip.getViciousHammers());
                        ps.setString(28, c.getPlayer().getName());
                        ps.setTimestamp(29, endDate);
                    }
                    ps.executeUpdate();
                    ps.close();
                    MapleInventoryManipulator.removeFromSlot(c, type, position, quantity, false);
                } catch (SQLException e) {
                    log.error("MTS Sale error: " + e);
                }

                c.sendPacket(MaplePacketCreator.MTSConfirmSell());
                c.sendPacket(getMTS(1, 0, 0));
                c.sendPacket(MaplePacketCreator.enableMTS());
                c.sendPacket(MaplePacketCreator.transferInventory(getTransfer(c.getPlayer().getId())));
                c.sendPacket(MaplePacketCreator.notYetSoldInv(getNotYetSold(c.getPlayer().getId())));
                ...
                try {
                    ...
                    if (rs.next()) {
                        IItem i;
                        if (rs.getInt("type") != 1) {
                            Item item = new Item(rs.getInt("itemid"), (byte) 0, rs.getShort("quantity"), rs.getShort("flag"));
                            item.setOwner(rs.getString("owner"));
                            item.setExpiration(rs.getTimestamp("ExpireDate"));
                            item.setPosition(c.getPlayer().getInventory(MapleItemInformationProvider.getInstance().getInventoryType(rs.getInt("itemid"))).getNextFreeSlot());
                            i = item.copy();
                        } else {
                        ...
                        c.sendPacket(MaplePacketCreator.modifyInventory(true, MapleInventoryManipulator.addByItem(c, i, "MTS transfer", false)));

                        c.sendPacket(MaplePacketCreator.enableMTS());
                        c.sendPacket(getCart(c.getPlayer().getId()));
                        c.sendPacket(getMTS(c.getPlayer().getCurrentTab(), c.getPlayer().getCurrentType(), c.getPlayer().getCurrentPage()));
                        c.sendPacket(MaplePacketCreator.MTSConfirmTransfer(i.getQuantity(), i.getPosition()));
                        c.sendPacket(MaplePacketCreator.transferInventory(getTransfer(c.getPlayer().getId())));
                ...
                ResultSet rs;
                try {
                    ps = con.prepareStatement("SELECT * FROM mts_items WHERE id = ? AND transfer = 0 ORDER BY id DESC");
                    ps.setInt(1, id);

                    rs = ps.executeQuery();
                    if (rs.next()) {
                        int price = rs.getInt("price") + 100 + (int) (rs.getInt("price") * 0.1); //taxes
                        if (c.getPlayer().getCashShop().getCash(4) >= price && c.getPlayer().getId() != rs.getInt("seller")) {
                            boolean alwaysnull = true;
                            for (ChannelServer cserv : ChannelServer.getAllInstances()) {
                                MapleCharacter victim = cserv.getPlayerStorage().getCharacterById(rs.getInt("seller")); // TODO: Potential unsafe. Buying from your own account causes pointloss.
                                if (victim != null) {
                            ...
                            if (alwaysnull) {
                                ...
                                if (rse.next()) {
                                    PreparedStatement psee = con.prepareStatement("UPDATE accounts SET gameCardCash = gameCardCash + ? WHERE id = ?");
                            ...
                            c.sendPacket(MaplePacketCreator.enableMTS());
                            c.sendPacket(getMTS(c.getPlayer().getCurrentTab(), c.getPlayer().getCurrentType(), c.getPlayer().getCurrentPage()));
                            c.sendPacket(MaplePacketCreator.MTSConfirmBuy());
                            c.sendPacket(MaplePacketCreator.showMTSCash(c.getPlayer()));
                            c.sendPacket(MaplePacketCreator.transferInventory(getTransfer(c.getPlayer().getId())));
                            c.sendPacket(MaplePacketCreator.notYetSoldInv(getNotYetSold(c.getPlayer().getId())));
                            c.sendPacket(MaplePacketCreator.enableActions());
                        } else {
                            c.sendPacket(MaplePacketCreator.MTSFailBuy());
                        }
                    } else { // Handle item not found
                        c.sendPacket(MaplePacketCreator.MTSFailBuy());
                    }
                    rs.close();
                    ps.close();
            ...
            while (rs.next()) {
                ...
                if (rse.next()) {
                    if (rse.getInt("type") != 1) {
                        Item i = new Item(rse.getInt("itemid"), (byte) 0, rse.getShort("quantity"), rse.getShort("flag"));
                        i.setOwner(rse.getString("owner"));
                        i.setExpiration(rse.getTimestamp("ExpireDate"));
                        items.add(new MTSItemInfo(i, rse.getInt("price"), rse.getInt("id"), rse.getInt("seller"), rse.getString("sellername"), rse.getTimestamp("sell_ends")));
                    } else {
                        ...
                        equip.setExpiration(rse.getTimestamp("ExpireDate"));
                        equip.setLevel((byte) rse.getInt("level"));
                        equip.setViciousHammers(rse.getInt("hammer"));
                        items.add(new MTSItemInfo(equip, rse.getInt("price"), rse.getInt("id"), rse.getInt("seller"), rse.getString("sellername"), rse.getTimestamp("sell_ends")));
                    }
                }
                rse.close();
                pse.close();
    ...
    public MaplePacket getMTSSearch(int tab, int type, int cOi, String search, int page) {
        ...
        if (cOi != 0) {
            ...
            listItems.append(" AND ( ");
            if (!retItems.isEmpty()) {
                for (String singleRetItem : retItems) {
                    listItems.append(singleRetItem);
                }
            }
            listItems.append("itemid = 0 )");
        } else {
            listItems.append(" AND sellername LIKE CONCAT('%','").append(search).append("', '%')");
        }
        ...
        ResultSet rs;
        int pages = 0;
        String sql = "SELECT * FROM mts_items WHERE tab = ?" + listItems.toString() + " ";
        try {
            if (type != 0) {
                sql += "AND type = ? AND transfer = 0 ORDER BY id DESC LIMIT ?, 16";
            } else {
                sql += "AND transfer = 0 ORDER BY id DESC LIMIT ?, 16";
            }
            ...
            while (rs.next()) {
                if (rs.getInt("type") != 1) {
                    ...
                    i.setOwner(rs.getString("owner"));
                    i.setExpiration(rs.getTimestamp("ExpireDate"));
                    items.add(new MTSItemInfo((IItem) i, rs.getInt("price"), rs.getInt("id"), rs.getInt("seller"), rs.getString("sellername"), rs.getTimestamp("sell_ends")));
                } else {
                    Equip equip = new Equip(rs.getInt("itemid"), (byte) rs.getInt("position"), -1);
                    equip.setOwner(rs.getString("owner"));
                    equip.setQuantity((short) 1);
                    ...
                    equip.setLevel((byte) rs.getInt("level"));
                    equip.setViciousHammers(rs.getInt("hammer"));
                    items.add(new MTSItemInfo((IItem) equip, rs.getInt("price"), rs.getInt("id"), rs.getInt("seller"), rs.getString("sellername"), rs.getTimestamp("sell_ends")));
                }
            }
            rs.close();
            ps.close();
            if (type != 0) {
                ps = con.prepareStatement("SELECT COUNT(*) FROM mts_items WHERE tab = ? " + listItems.toString() + " AND type = ? AND transfer = 0");
            } else {
                ps = con.prepareStatement("SELECT COUNT(*) FROM mts_items WHERE tab = ? " + listItems.toString() + " AND transfer = 0");
                ps.setInt(1, tab);
                rs = ps.executeQuery();
                if (rs.next()) {
                    pages = rs.getInt(1) / 16;
            ...
            log.error("MTS Getting search failed - SQL Statement = " + sql, e);
        }
        return MaplePacketCreator.sendMTS(items, tab, type, page, pages);
    }
}