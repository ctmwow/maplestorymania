...
public class BBSOperationHandler extends AbstractMaplePacketHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        if (!MapleGuild.ENABLE_BBS) {
            c.sendPacket(MaplePacketCreator.serverNotice(1, "Your server administrator has currently disabled Guild BBS."));
            return;
        }
    ...
    public static void listBBSThreads(MapleClient c, int start) {
        ...
        try {
            ...
            c.sendPacket(MaplePacketCreator.BBSThreadList(rs, start));
    ...
    public static void deleteBBSThread(MapleClient client, int localthreadid) {
        ...
        try {
            ...
            ps.setInt(2, localthreadid);
            ResultSet threadRS = ps.executeQuery();
            if (!threadRS.next() || mc.getId() != threadRS.getInt("postercid") && mc.getGuildRank() > 2) {
                ps.close();
                threadRS.close();
                return; // thread no longer exists, deleted?
            }
            ...
            PreparedStatement ps2 = con.prepareStatement("DELETE FROM bbs_replies WHERE threadid = ?");
            ps2.setInt(1, threadid);
            ps2.executeUpdate();
            ps2.close();

            PreparedStatement ps3 = con.prepareStatement("DELETE FROM bbs_threads WHERE threadid = ?");
            ps3.setInt(1, threadid);
            ps3.executeUpdate();
            ps3.close();
        } catch (SQLException se) {
            log.error("SQLException: " + se.getLocalizedMessage(), se);
    ...
    public static void deleteBBSReply(MapleClient client, int replyid) {
        ...
        try {
            ...
            ps.setInt(1, replyid);
            ResultSet rs = ps.executeQuery();
            if (!rs.next() || mc.getId() != rs.getInt("postercid") && mc.getGuildRank() > 2) {
                rs.close();
                ps.close();
            ...
            PreparedStatement ps2 = con.prepareStatement("DELETE FROM bbs_replies WHERE replyid = ?");
            ps2.setInt(1, replyid);
            ps2.executeUpdate();
            ps2.close();

            PreparedStatement ps3 = con.prepareStatement("UPDATE bbs_threads SET replycount = replycount - 1 WHERE threadid = ?");
            ps3.setInt(1, threadid);
            ps3.executeUpdate();
            ps3.close();
    ...
    public static void displayThread(MapleClient client, int threadid, boolean bIsThreadIdLocal) {
        ...
        try {
            ...
            client.sendPacket(MaplePacketCreator.showThread(bIsThreadIdLocal ? threadid : threadRS.getInt("localthreadid"), threadRS, repliesRS));
            threadRS.close();
        } catch (SQLException se) {
            log.error("SQLException: " + se.getLocalizedMessage(), se);
