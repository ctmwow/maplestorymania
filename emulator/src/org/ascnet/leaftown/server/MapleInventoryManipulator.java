...
import org.ascnet.leaftown.client.ItemFlag;
...
public class MapleInventoryManipulator 
{
    ...
    public static void addById(MapleClient c, int itemId, short quantity, String logInfo, String owner, MaplePet pet, boolean gm) 
    {
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) 
        {
            ...
            if (!ii.isThrowingStar(itemId) && !ii.isShootingBullet(itemId)) 
            {
                ...
                while (quantity > 0x00)// add new slots if there is still something left
                { 
                    short newQ = (short) Math.min(quantity, slotMax);
                    if (newQ != 0) 
                    {
                        quantity -= newQ;
                        Item nItem = new Item(itemId, (short) 0x00, newQ);
                        ...
                        if ((ii.isThrowingStar(itemId) || ii.isShootingBullet(itemId)) && quantity == 0x00)
                            break;
                    } 
                    else 
                    {
            ...
            { 
                Item nItem = new Item(itemId, (short) 0x00, quantity);
                if (gm)
                    nItem.setGMFlag();
    ...
    public static List<Pair<Short, IItem>> addByItem(MapleClient c, IItem item, String logInfo, boolean pickUp)
    {
        ...
        if (quantity < 0)
            return items;
        if (pickUp && ii.canHaveOnlyOne(item.getItemId()) && c.getPlayer().haveItem(item.getItemId(), 0x01, true, true)) 
        {
            c.sendPacket(MaplePacketCreator.getInventoryFull());
        ...
        if (!type.equals(MapleInventoryType.EQUIP))
        {
            ...
            if (!ii.isThrowingStar(item.getItemId()) && !ii.isShootingBullet(item.getItemId()))
            {
                        ...
                        if (i.hasNext())
                        {
                            ...
                            if (oldQ < slotMax && item.isFromOwner(eItem.getOwner()) && item.getExpiration().compareTo(eItem.getExpiration()) == 0x00)
                            {
                                ...
                                if (item.isByGM())
                                    eItem.setGMFlag();
                                eItem.log("Added " + (newQ - oldQ) + " items to stack, new quantity is " + newQ + " (" + logInfo + " )", false);
                                items.add(new Pair<Short, IItem>((short) 1, eItem));
                            }
                        } 
                        else break;
                    }
                ...
                while (quantity > 0x00)// add new slots if there is still something left
                {
                    short newQ = (short) Math.min(quantity, slotMax);
                    quantity -= newQ;
                    Item nItem = new Item(item.getItemId(), (byte) 0x00, newQ);
                    ...
                    items.add(new Pair<Short, IItem>((short) 0x00, nItem));
                }
            }
            else 
            {
                // Throwing Stars and Bullets - Add all into one slot regardless of quantity.
                Item nItem = new Item(item.getItemId(), (byte) 0x00, quantity);
                nItem.setOwner(item.getOwner());
                nItem.setExpiration(item.getExpiration());
                nItem.log("Created while adding by id. Quantity: " + quantity + " (" + logInfo + " )", false);
        ...
        {
            if (!logInfo.contains("Canceled trade") && !logInfo.contains("returned from shop") && (item.getFlag() & ItemFlag.TRADE_ONCE.getValue()) == 0x10)
            ...
            items.add(new Pair<>((short) 0x00, item));
        }
        if (pickUp) 
        { 
            c.sendPacket(MaplePacketCreator.modifyInventory(true, items));
    ...
    public static boolean checkSpace(MapleClient c, int itemid, int quantity, String owner)
    {
        ...
        if (!type.equals(MapleInventoryType.EQUIP))
        {
            ...
            if (slotMax > 0x00) // add new slots if there is still something left
            ...
            return !c.getPlayer().getInventory(type).isFull(numSlotsNeeded - 0x01);
        }
        else 
    ...
    {
    	removeFromSlot(c, type, slot, quantity, fromDrop, consume, true);
    }

    public static void removeFromSlot(MapleClient c, MapleInventoryType type, short slot, int quantity, boolean fromDrop, boolean consume, boolean sendUpdate) 
    {
        if (quantity < 0x01)
            return;
    ...
    {
        if (quantity < 0x00)
            return new Pair<>((short) -0x01, null);
        ...
        {
            return new Pair<>((short) 0x03, item);
        }
        else 
        {
            if (!consume) 
            ...
            return new Pair<>((short) 0x01, item);
        }
    }
    ...
    public static void equip(MapleClient c, short src, short dst) {
        ...
        if (!ii.isCashItem(itemId)) {
            final boolean isCorrectSlot;
            switch (dst) {
                case -1:
                    isCorrectSlot = type.equals("Cp");
                    break;
                case -2:
                    isCorrectSlot = type.equals("Af");
                    break;
                case -3:
                    isCorrectSlot = type.equals("Ay");
                    break;
                case -4:
                    isCorrectSlot = type.equals("Ae");
                    break;
                case -5:
                    isCorrectSlot = type.startsWith("Ma");
                    break;
                case -6:
                    isCorrectSlot = type.equals("Pn");
                    break;
                case -7:
                    isCorrectSlot = type.equals("So");
                    break;
                case -8:
                    isCorrectSlot = type.equals("Gv");
                    break;
                case -9:
                    isCorrectSlot = type.equals("Sr");
                    break;
                case -10:
                    isCorrectSlot = type.equals("Si");
                    break;
                case -11:
                    isCorrectSlot = type.startsWith("Wp");
                    break;
                case -12:
                case -13:
                case -15:
                case -16:
                    isCorrectSlot = type.equals("Ri");
                    break;
                case -17:
                    isCorrectSlot = type.equals("Pe");
                    break;
                case -18:
                    isCorrectSlot = type.equals("Tm");
                    break;
                case -19:
                    isCorrectSlot = type.equals("Sd");
                    break;
                case -49:
                    isCorrectSlot = type.equals("Me");
                    break;
                case -50:
                    isCorrectSlot = type.equals("Be");
                    break;
                default:
                    isCorrectSlot = true;
                    break;
