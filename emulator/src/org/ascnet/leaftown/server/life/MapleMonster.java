...
import org.ascnet.leaftown.tools.ArrayMap;
        ...
        public void run() {
            ...
            if (damage >= hp) {
                ...
                if (!shadowWeb && !ninjaAmbush) {
                    ...
                    }, status.getSkill().getEffect(chr.getSkillLevel(status.getSkill())).getDuration());
                }
            }
            if (hp > 1 && damage > 0) {
                if (ninjaAmbush) {
            ...
            for (Entry<MapleCharacter, OnePartyAttacker> attacker : attackers_.entrySet()) {
                ...
                for (MaplePartyCharacter partychar : party.getMembers()) {
                    if (attacker.getKey().getLevel() - partychar.getLevel() <= 5 || getLevel() - partychar.getLevel() <= 5) {
                        ...
                        if (pchr != null) {
                            if (pchr.isAlive() && pchr.getMap() == map && !pchr.getCashShop().isOpened() && !pchr.inMTS()) {
